 ...
 */
export const purgeDeletedUsers = pubsub
  ...
  .onRun(async () => {
    ...
    const snap = await db
      .collection("users")
      .where("status", "==", STATUS_PENDING_DELETION)
      .where("purgeAt", "<=", now)
      .limit(MAX_USERS_PER_RUN)
      .get();

    if (snap.empty) {
      logger.info("purgeDeletedUsers: 파기 대상 없음");
      return;
    }
    ...
    for (const doc of snap.docs) {
      try {
        ...
        logger.error(`purgeDeletedUsers: uid=${doc.id} 파기 실패`, e);
      }
    }

    logger.info(
      `purgeDeletedUsers: ${purged}/${snap.size} 파기 완료`
    );
  });
...
async function purgeUser(uid: string): Promise<void> {
  ...
  for (const doc of reviews.docs) {
    ...
    if (clubId) {
      await deleteStorageFolder(`reviews/${clubId}/${doc.id}/`);
    }
  }
  await commitInChunks(reviews.docs, (batch, doc) => batch.delete(doc.ref));
  ...
  const photos = await db
    ...
    .where("userId", "==", uid)
    .get();
  await commitInChunks(photos.docs, (batch, doc) => batch.delete(doc.ref));
  ...
  const favorites = await db
    ...
    .where("userId", "==", uid)
    .get();
  await commitInChunks(favorites.docs, (batch, doc) => batch.delete(doc.ref));
  ...
  const history = await db
    ...
    .get();
  await commitInChunks(history.docs, (batch, doc) => batch.delete(doc.ref));
  ...
  try {
    ...
    logger.warn(`purgeUser: Auth 삭제 건너뜀 uid=${uid}`, e);
  }

  logger.info(
    `purgeUser: uid=${uid} reviews=${reviews.size} photos=${photos.size} ` +

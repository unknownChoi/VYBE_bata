...
class _HipHopScreenState extends ConsumerState<HipHopScreen> {
  ...
  List<_HipClub> _grid(List<_HipClub> source) {
    final base =
        _area == '인기순' ? source : source.where((c) => c.area == _area).toList();
  ...
  Widget build(BuildContext context) {
    ...
    final heroSrc = (data?.headlinerByClub.values.toList() ?? <PerformanceModel>[])
      ..sort((a, b) {
        final f = (b.isFeatured ? 1 : 0) - (a.isFeatured ? 1 : 0);
        return f != 0 ? f : a.startAt.compareTo(b.startAt);
      });
    final heroes =
        heroSrc.map((p) => _heroFromPerf(p, clubById[p.clubId])).toList();
    // 아티스트 = 오늘 공연 전체(시작시각순).
    final djs = [for (var i = 0; i < perfs.length; i++) _djFromPerf(perfs[i], i)];

    // 그리드(포스터) = 클럽 + 오늘 라인업 머지(live/lineup).
    final source =
        clubs.map((c) => _fromClub(c, headliner[c.clubId])).toList();
              ...
              child: ListView(
                ...
                children: [
                  if (loading)
                    ...
                    _HeroCarousel(
                        heroes: heroes, saved: _saved, onSave: _toggleSave)
                  else
                    const _HeroEmpty(),
                  // 오늘의 공연 아티스트 (항상 노출)
                  Padding(
                    padding: EdgeInsets.only(top: 24.h),
                    child: const _SectionHead(
                      title: '오늘의 공연 아티스트',
                        ...
                        Expanded(
                          child: Text(
                            ...
                            style: VybeTypography.caption.copyWith(
                                height: 17 / 12, color: VybeColors.gray400),
                          ),
                        ),
                      ],
                    ),
                    ...
                    Padding(
                      ...
                      child: GridView.count(
                        ...
                        children: List.generate(
                            4, (_) => VybeSkel(height: double.infinity, radius: 16)),
                      ),
                    )
...
class _Header extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return AnimatedContainer(
      ...
      decoration: BoxDecoration(
        color: scrolled ? const Color(0xFF101013).withValues(alpha: 0.9) : Colors.transparent,
        border: Border(
          bottom: BorderSide(
        ...
        children: [
          ...
          AnimatedOpacity(
            ...
            child: Row(
              ...
              children: [
                Icon(Icons.mic_none_rounded, size: 17.r, color: _hip),
                SizedBox(width: 6.w),
                Text('힙합',
          ...
          SizedBox(
            ...
            child: IconButton(
              onPressed: () {},
              icon: Icon(Icons.search_rounded, size: 21.r, color: Colors.white),
            ),
          ),
        ],
      ),
      ...
      child: Column(
        ...
        children: [
          Container(
            ...
            child: Icon(Icons.mic_external_off_rounded, size: 26.r, color: _hip),
          ),
          ...
          Text(
            ...
            style: VybeTypography.body4.copyWith(
                height: 18 / 13, color: VybeColors.gray500),
          ),
        ],
      ),
    );
...
class _HeroCarousel extends StatefulWidget {
  ...
  const _HeroCarousel(
      {required this.heroes, required this.saved, required this.onSave});
              ...
              children: [
                for (var i = 0; i < heroes.length; i++)
                  AnimatedContainer(
                    ...
                    decoration: BoxDecoration(
                      color: i == _active ? _hip : Colors.white.withValues(alpha: 0.4),
...
class _HeroSlide extends StatelessWidget {
  ...
  const _HeroSlide({required this.h, required this.saved, required this.onSave});
          ...
          child: DecoratedBox(
            decoration: BoxDecoration(
              gradient: LinearGradient(
                ...
                colors: [Color(0xFF0D0A0C), Color(0x8C0D0A0C), Color(0x260D0A0C), Color(0x660D0A0C)],
            ...
            children: [
              ...
              Text(
                ...
                style: TextStyle(
                  ...
                  color: Colors.white,
                  shadows: const [
                    Shadow(color: Color(0x800D0A0C), blurRadius: 24, offset: Offset(0, 2)),
                  ],
                ),
              ),
                ...
                children: [
                  ...
                  Text('${h.lineup} LIVE',
                      style: VybeTypography.body4
                          .copyWith(fontWeight: FontWeight.w700, color: _hip)),
                  SizedBox(width: 8.w),
                  _dot(),
                  SizedBox(width: 8.w),
                  Text(h.genre,
                      style: VybeTypography.body4.copyWith(
                          fontWeight: FontWeight.w600, color: VybeColors.gray200)),
                ],
              ),
              SizedBox(height: 16.h),
              // CTA.
              Container(
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    Text('지금 입장 정보 보기',
                        style: VybeTypography.button1
                            .copyWith(fontWeight: FontWeight.w800, color: _onHip)),
                  ],
                ),
              ),
              ...
              Row(
                children: [
                  ...
                  SizedBox(width: 4.w),
                  Text(h.rating.toStringAsFixed(2),
                      style: VybeTypography.body4
                          .copyWith(fontWeight: FontWeight.w700, color: Colors.white)),
                  SizedBox(width: 7.w),
                  _dot(),
                  SizedBox(width: 7.w),
                  Icon(Icons.place_rounded, size: 12.r, color: VybeColors.gray300),
                  SizedBox(width: 4.w),
                  Text('${h.area} · ${h.dist}km',
                      style: VybeTypography.body4.copyWith(color: VybeColors.gray300)),
                  SizedBox(width: 7.w),
                  _dot(),
                  SizedBox(width: 7.w),
                  Text(h.time,
...
class _SectionHead extends StatelessWidget {
  ...
  const _SectionHead({required this.title, required this.sub, this.mapAction = false});

  @override
  Widget build(BuildContext context) {
    return Padding(
      padding: EdgeInsets.only(left: 16.w, right: 16.w, bottom: 13.h),
      child: Row(
          ...
          if (mapAction)
            ...
            Row(
              ...
              children: [
                ...
                Icon(Icons.chevron_right_rounded, size: 14.r, color: VybeColors.gray500),
              ],
            ),
        ],
      ),
      ...
      child: Column(
        children: [
          ...
          Text(
            ...
            style: VybeTypography.caption.copyWith(
                height: 14 / 12, fontWeight: FontWeight.w700, color: Colors.white),
          ),
          SizedBox(height: 2.h),
          Text(
            '${d.isDj ? 'DJ · ' : ''}${d.club}',
                ...
                children: [
                  if (s != '인기순') ...[
                    Icon(Icons.place_rounded,
                        size: 12.r, color: sel ? _onHip : VybeColors.gray400),
                    SizedBox(width: 5.w),
                  ],
                  Text(
                    s,
                    style: VybeTypography.button2.copyWith(
                        fontWeight: sel ? FontWeight.w700 : FontWeight.w500, color: fg),
                  ),
                ],
              ),
            ),
...
class _PosterCard extends StatelessWidget {
  ...
  final bool saved;
  final VoidCallback onSave;
  const _PosterCard({required this.club, required this.saved, required this.onSave});
                ...
                children: [
                  ...
                  Row(
                    children: [
                      ...
                      SizedBox(width: 4.w),
                      Text('${club.area} · ${club.dist.toStringAsFixed(1)}km',
                          style: VybeTypography.caption.copyWith(
                              fontSize: 11.sp,
                              height: 12 / 11,
                              fontWeight: FontWeight.w600,

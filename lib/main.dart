...
class VybeApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ScreenUtilInit(
      ...
      // 게이트를 만들지 않는다. 버전 체크·세션 복원은 그 동안 병렬로 돌아간다
      // (SplashGate가 splashDestinationProvider를 통해 앱 시작 시 바로 건다).
      // 버전 게이트가 인증보다 위 — 로그인 여부와 무관하게 먼저 걸러야 하고,
      // 강제 업데이트/점검 차단이 로그인 플로우 뒤로 밀리면 우회가 가능해진다.
      // 비로그인이면 AuthGate가 MainScaffold 진입 자체를 막는다.
      // 네트워크 게이트가 버전보다 위 — 연결이 없으면 버전 조회부터 실패하므로
      // '업데이트 안내'가 아니라 '연결 확인' 안내를 보여 줘야 한다.
      child: const SplashGate(
        child: NetworkGate(child: VersionGate(child: AuthGate())),
      ),
    );
  }
}
